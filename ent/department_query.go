...
// Returns a *NotSingularError when exactly one Department entity is not found.
...
// Returns a *NotSingularError when exactly one Department ID is not found.
...
func (dq *DepartmentQuery) sqlAll(ctx context.Context) ([]*Department, error) {
	...
	if query := dq.withEmployees; query != nil {
		...
		for _, n := range neighbors {
			...
			if fk == nil {
				return nil, fmt.Errorf(`foreign-key "department_employees" is nil for node %v`, n.ID)
			}
			node, ok := nodeids[*fk]
			if !ok {
				return nil, fmt.Errorf(`unexpected foreign-key "department_employees" returned %v for node %v`, *fk, n.ID)
			}
